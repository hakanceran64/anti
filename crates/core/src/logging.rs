...
pub fn init_logging(config: &LoggingConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Windows Event Log layer (Windows-specific)
    #[cfg(windows)]
        ...
        .try_init()
    {
        ...
        Err(e) => {
            ...
            if error_msg.contains("global default trace dispatcher has already been set") {
                // This is fine, logging is already initialized
                Ok(())
            } else {
                Err(Box::new(e))
            }
...
/// Log a security event with structured data (legacy function for compatibility)
pub fn log_security_event(
    ...
    // Log using the legacy format for backward compatibility
    match severity {
...
/// Create and log a structured security event
...
/// Create and log a structured scan event
...
/// Create and log a structured threat detection event
...
/// Create and log a structured quarantine event
...
/// EventLogger struct for comprehensive logging and audit functionality
...
impl EventLogger {
    ...
    /// Log a structured event with JSON support
...
/// Log rotation and archiving functionality
...
impl LogRotator {
    ...
    fn rotate_log(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Clean up old rotated files if we exceed max_files
    ...
    pub fn archive_old_logs(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for entry in entries {
            ...
            if path.is_file() && path.extension().map_or(false, |ext| ext == "log") {
                ...
                // For now, we'll just rename it to indicate it's archived
    ...
    /// Remove excess rotated files beyond max_files limit
...
/// Structured event for comprehensive logging
...
/// Event severity levels for structured logging
...
/// Windows Event Log layer for tracing (Windows-specific)
#[cfg(windows)]
        ...
        if let Some(handle) = self.event_handle {
            ...
            unsafe {
                let _ = ReportEventW(
                    ...
                    event_type,
                    0, // Category
                    1001, // Event ID for tracing events
                    None, // User SID
                    &strings,
                    None, // Raw data
                );
...
/// Audit logger for compliance and forensic purposes
...
mod tests {
    ...
    fn test_log_rotation_check() {
        ...
        let config = LoggingConfig {
            log_level: "info".to_string(),
            log_file_path: log_path.clone(),
            max_log_file_size_mb: 1, // 1 MB limit
            max_log_files: 3,
            enable_console_logging: false,
            enable_windows_event_log: false,
            enable_json_logging: true,
        ...
        // Create a small log file (should not trigger rotation)
        fs::write(&log_path, "small log content").unwrap();
        assert!(rotator.check_and_rotate().is_ok());
        assert!(log_path.exists());

        // The file is too small to trigger rotation, so it should still exist
        assert!(log_path.exists());
    }
    ...
    fn test_structured_event_logging() {
        ...
        // Check that event counter was incremented
        let stats = event_logger.get_event_statistics();
    ...
    fn test_log_cleanup() {
        ...
        // Test cleanup with 0 days (should remove all files)
