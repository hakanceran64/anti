/// File utilities for scanning operations
...
impl FileScanner {
    ...
    /// Set buffer size for reading files
    ...
    /// Calculate SHA-256 hash only (faster for signature matching)
    ...
    where
        F: FnMut(&[u8]) -> Result<bool>, // Return false to stop reading
    {
        let file = async_fs::File::open(path).await?;
        let mut reader = AsyncBufReader::with_capacity(self.buffer_size, file);
    ...
    pub async fn get_file_signature(&self, path: &Path) -> Result<Vec<u8>> {
        ...
        let mut signature = vec![0u8; 16]; // Read first 16 bytes
    ...
    pub async fn is_pe_file(&self, path: &Path) -> Result<bool> {
        ...
        if signature.len() >= 2 && signature[0] == 0x4D && signature[1] == 0x5A {
            // Read more to check for PE signature
    ...
    /// Extract strings from file (for analysis)
...
/// Directory walker for recursive scanning
...
impl DirectoryWalker {
    ...
    /// Walk directory with callback for each file
    pub async fn walk_with_callback<F>(&self, root: &Path, mut callback: F) -> Result<()>
    where
        F: FnMut(&Path) -> Result<bool>, // Return false to stop walking
    {
    ...
    /// Recursive directory walking with callback
...
mod tests {
    ...
    async fn test_pe_file_detection() {
        ...
        pe_content.extend_from_slice(&[0x50, 0x45, 0x00, 0x00]); // PE\0\0
