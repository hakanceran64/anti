...
async fn test_analyze_file_success() {
    ...
    // Analysis might fail on non-Windows systems, but the service should handle it gracefully
...
async fn test_file_size_limits() {
    ...
    // Create a large test file (larger than default limit)
    let temp_dir = TempDir::new().unwrap();
    let large_file = temp_dir.path().join("large.bin");
    let mut file = File::create(&large_file).unwrap();
    
    // Write more than 512MB (default limit)
    let large_data = vec![0u8; 1024 * 1024]; // 1MB chunk
    for _ in 0..600 { // Write 600MB
