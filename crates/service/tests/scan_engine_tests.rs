...
async fn test_eicar_detection() {
    ...
    // Should detect EICAR if signatures are loaded
...
async fn test_directory_scanning() {
    ...
    // Start custom scan of the directory
...
async fn test_scan_statistics() {
    ...
    // Statistics should be updated after scanning
...
async fn test_signature_database_integration() {
    ...
    // Should have some basic information even if no signatures loaded
    assert!(!sig_stats.version.is_empty());
    
    // Try to reload signatures
    let reload_result = engine.reload_signatures().await;
    // This might fail if no signature files exist, which is OK for testing
    match reload_result {
        Ok(stats) => {
            assert!(stats.compilation_time_ms >= 0);
        }
        Err(_) => {
            // No signature files found, which is acceptable in test environment
        }
    }
}
...
async fn test_file_scanner_configuration() {
    ...
    // File scanner should be configured with reasonable defaults
    
    let directory_walker = engine.get_directory_walker();
    // Directory walker should be configured with reasonable defaults
    
    // These are just basic checks that the components exist
    // More detailed testing is done in the individual component tests
}
...
async fn test_executable_file_analysis() {
    ...
    // Create a fake executable file with PE header
    ...
    // Should detect suspicious strings in executable
