// Standalone tests for configuration manager that don't depend on other service modules
use hadron_core::{
...
fn test_whitelist_entry_addition() {
    ...
    let entry = WhitelistEntry {
        ...
        hash: Some("a1b2c3d4e5f67890123456789abcdef0".to_string()), // 32 chars (MD5)
...
fn test_expired_whitelist_cleanup() {
    ...
    // Add one expired entry and one valid entry
    ...
    // Manually add to cache to bypass validation
    {
...
async fn test_config_change_notification() {
    ...
    // Start waiting for config change in background
