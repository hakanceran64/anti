...
/// Events from background scan operations
#[derive(Debug, Clone)]
enum ScanEvent {
    Started(ScanJobId),
...
/// Application state for different operations
...
impl AntivirusApp {
    pub fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        ...
        // Try to connect to service in background
        ...
        // Load initial removable devices immediately
    ...
    fn handle_scan_action(&mut self, action: ScanPanelAction) {
        match action {
            ...
            ScanPanelAction::StartFlashDriveScan => {
                ...
                // Redirect to removable media panel
                self.selected_panel = Panel::RemovableMedia;
            }
    ...
    fn start_scan_async(&mut self, scan_type: ScanType, targets: Vec<std::path::PathBuf>) {
        ...
        // Use channels for communication back to main thread
    ...
    fn cancel_scan_async(&mut self, scan_id: ScanJobId) {
        ...
        std::thread::spawn(move || {
            runtime.block_on(async {
                // Note: We need to implement cancel_scan in the API client
    ...
    /// Start a flash scan with the specified type
    fn start_flash_scan(&mut self, scan_type: FlashScanType) {
        ...
        let scan_type_str = match scan_type {
            ...
            FlashScanType::NetworkScan => {
                ...
                ScanType::Quick // Use Quick as fallback since Network doesn't exist
            }
        };
    ...
    fn add_mock_scan_result(&mut self) {
        ...
        let mock_threats = vec![
            ThreatInfo::new(
                ...
                // Fallback if ThreatInfo::new fails
                ThreatInfo {
                    id: uuid::Uuid::new_v4(),
        ...
        self.scan_statistics.files_scanned += 1000; // Mock value
    ...
    fn update_system_performance(&mut self) {
        ...
        // Add some suspicious activity occasionally
    ...
    fn save_settings_async(&mut self) {
        ...
        std::thread::spawn(move || {
            runtime.block_on(async {
                // Note: We need to implement configuration update in the API client
    ...
    fn update_scan_progress(&mut self) {
        if let Some(scan_id) = self.current_scan_id {
            ...
            // Create a temporary channel for progress updates if we have an active scan
            let (tx, rx) = std::sync::mpsc::channel();
            ...
            // Process any immediate progress updates
    ...
    pub fn simulate_scan_completion(&mut self, threats_found: u32, files_scanned: u64) {
        ...
        self.scan_panel.set_scanning(false);
        self.removable_media_panel.set_scanning(false); // Also stop removable media scanning
        self.current_scan_id = None;
        self.status_message = format!("Scan completed - {} threats found in {} files", threats_found, files_scanned);
    }

    /// Process scan events from background threads
            ...
            while let Ok(event) = receiver.try_recv() {
                match event {
                    ...
                    ScanEvent::Completed(result) => {
                        ...
                        // Add threat notifications for each detected threat
    ...
    /// Process system status events from background threads
    ...
    /// Process removable devices events from background threads
    ...
    /// Process removable device scan results from background threads
                ...
                match result {
                    Ok(scan_result) => {
                        ...
                        if threats_count > 0 {
                            ...
                            // Add notification for each threat found
                            ...
                            // Add scan completion notification
                            self.notification_manager.add_notification(
    ...
    /// Process all devices scan results from background threads
    ...
    fn is_service_connected(&self) -> bool {
        // In a real implementation, we'd check the actual connection status
        // For now, we'll simulate based on whether we have a recent status update
    ...
    fn scan_all_removable_devices(&mut self) {
        ...
        // Create channel for all scan results
        let (tx, rx) = std::sync::mpsc::channel();
    ...
    /// Clean removable device (scan and remove threats)
    ...
    /// Refresh removable devices list
    fn refresh_removable_devices(&mut self) {
        ...
        // Create channel for communication back to main thread
        let (tx, rx) = std::sync::mpsc::channel();
    ...
    /// Update removable media devices periodically
    fn update_removable_media(&mut self) {
        let now = std::time::Instant::now();
        
        // If we're on the removable media panel and haven't updated recently
    ...
    /// Load initial removable devices synchronously
    fn load_initial_removable_devices(&mut self) {
        let client = self.api_client.clone();
        let runtime = self.runtime.clone();
        
        // Try to load devices synchronously with a timeout
...
impl eframe::App for AntivirusApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Show main UI - simplified for testing
        egui::CentralPanel::default().show(ctx, |ui| {
        ...
        // Request repaint for animations and updates
